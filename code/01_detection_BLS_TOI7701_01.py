...
Includes a Savitzky-Golay window sensitivity analysis to justify depths, 
robust epoch assignment for odd/even vetting, and global transit masking.
Exports phase-folded light curve data for downstream statistical validation.
...
OUTPUT:
    ...
    - TOI7701.01_LC_folded_binned.csv (30-min binned for plotting)
    - TOI7701.01_LC_folded_raw.csv    (Full cadence, unbiased master for TRICERATOPS)
"""
...
fname_binned = f"{TOI_ID.replace(' ', '')}_LC_folded_binned.csv"
...
fname_raw = f"{TOI_ID.replace(' ', '')}_LC_folded_raw.csv"
